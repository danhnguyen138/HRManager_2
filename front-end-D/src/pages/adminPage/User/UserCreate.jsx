...
const createEmployeeSchema = yup.object().shape(
    {
        ...
        idChucVu: yup.string().required('Xin chọn chức vụ'),
...
const CreateUser = () => {
    ...
    const navigate = useNavigate();// Link liên kết 
    const token = useSelector((state) => state.token);//Lấy token
    const [selectedFile, setSelectedFile] = useState(null);// chọn file ảnh
    const { showNotification } = useStateContext(); //show thông báo
    ...
    let dataPosition = []; //Biến lưu giá trị chức vụ
                                        ...
                                        <form className="bg-white w-full shadow-md p-3 rounded" onSubmit={handleSubmit}>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 md:gap-6">
                                                <div className='mb-4'>
                                                    ...
                                                    {errors.idPhongBan && touched.idPhongBan ? (<div className='text-sm text-red-700 mb-1'>{errors.idPhongBan}</div>) :
                                                        (<div className='mb-3'></div>)
                                                    }
                                                    <Field
                                                ...
                                                <div className='mb-4'>
                                                    <label className=' block tetx-gray-700 font-bold mb-2'>Chức vụ</label>
                                                    {errors.idChucVu && touched.idChucVu ? (<div className='text-sm text-red-700 mb-1'>{errors.idChucVu}</div>) :
                                                        (<div className='mb-3'></div>)
                                                    }
                                                    <Field
                                                        as="select" name='idChucVu' value={values.idChucVu} className="shadow appearance-none border border-slate-950 rounded-lg w-full p-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline focus:border-blue-500"
                                                    >
                                                        <option value=''>
                                                            ---Lựa chọn chức vụ---
                                                        </option>
                                                        {
                                                            getDataPosition.getCV.map((item, index) => {
                                                                return (<option value={item.id} key={item.TenChucVu + index}>{item.TenChucVu}</option>)
                                                            })
                                                        }
                                                    </Field>
                                                </div>
