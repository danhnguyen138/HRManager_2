...
const Insurance = () => {
  ...
  //Lay Lich su cap nhat
  const [getDataHistory, setGetDataHistory] = useState(null);
  const getHistory = async () => {
    await fetch(`/api/user/baohiem/lichsucapnhat/${user.id}`, {
      method: 'GET',
      headers: {
  ...
  const getInsurance = async () => {
    await fetch(`/api/user/insurance/${user.id}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`
      }
  ...
  const getEmployee = async () => {
    await fetch(`/api/user/danhsachnhanvien/${user.id}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`
      },
  ...
  const getContract = async () => {
    await fetch(`/api/user/hopdong/${user.id}/detail`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`
      }
  ...
  const handleSubmit = async (values) => {
    await fetch(`/api/user/insurance/${user.id}/update`, {
      method: 'PUT',
      headers: {
        ...
        <div className='p-4 md:px-4 px-1'>
          <div className='mb-10 flex items-center justify-between gap-6'>
            <div className="flex items-center gap-6">
              ...
              <div>
                <h5 className="block antialiased tracking-normal font-sans text-xl font-semibold leading-snug mb-1">Thông tin bảo hiểm của {user.HoTen}</h5>
              </div>
